...
`;const Ee=({url:e})=>{fetch(e+"?t="+Date.now()).then((e=>e.json())).then((e=>{const n=e.files.map((t=>(/\.md$/.test(t)||(t+=".md"),fetch(e.path+t+"?version="+e.version||"0.0.0").then((e=>e.text())))));Promise.all(n).then((n=>{const a=[];n.forEach(((t,n)=>{a.push({name:e.files[n],text:t})})),t.innerHTML="",e.list=a,t.replaceWith(aoife(De,i({data:a},e)))}))}));const t=aoife("div",{class:"aoife-markdown-loading"},"Loading...");return t};V`
